...
use strict;
use warnings;
use base 'Lifecycle::Core::Agent';
use Lifecycle::Core::JobManager;
...
sub _default {
  ...
  Lifecycle::Core::Agent::_default(@_);
}
...
sub ReadConfig {
  ...
  Lifecycle::Util::ReadConfig($self,$hash,$file);
...
sub poe_default {
  ...
  if ( $workflow->{Module}{$event} ) {
    ...
    if ( !($loader = $self->{LOADER}{$namespace}) ) {
      $loader = $self->{LOADER}{$namespace} = Lifecycle::Core::Loader->new( NAMESPACE => $namespace );
    }
    eval {
