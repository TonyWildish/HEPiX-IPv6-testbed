...
Lifecycle::Core::Config - Manage Agent configuration, manipulate Agents directly
...
The Lifecycle::Core::Config object can read a PhEDEx config file and parse its
syntax. This syntax is not documented here, see the wiki probably (reference,
anyone?). As it parses the file it creates
L<Lifecycle::Core::Config::Agent|Lifecycle::Core::Config::Agent> and
L<Lifecycle::Core::Config::Environment|Lifecycle::Core::Config::Environment> objects
...
is probably 'perl -MLifecycle::Debug -d'.
...
L<Lifecycle::Core::Config::Agent|Lifecycle::Core::Config::Agent> objects that
...
L<Lifecycle::Core::Config::Environment|Lifecycle::Core::Config::Environment> with
that name. Recursively finds the parent environments, see the
L<Lifecycle::Core::Config::Environment|Lifecycle::Core::Config::Environment>
...
=head1 EXAMPLES
  ...
  my $config = Lifecycle::Core::Config->new();
